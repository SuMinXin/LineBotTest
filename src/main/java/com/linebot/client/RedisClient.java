...
public class RedisClient {
  ...
  public Map<String, List<String>> retrieveByPattern(String pattern) {
    ...
    try {
      ...
      for (String key : keys) {
        result.put(key, retrieveHashToken(key));
      }
    } catch (Exception e) {
      LOGGER.error("retrieveByPattern Exception, msg={}", e.getMessage());
    } finally {
      if (jedis != null)
        jedis.close(); // 歸還資源給JedisPool
    }
