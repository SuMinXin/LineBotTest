...
public class GoogleBotService extends AbstractService {
		...
		try {
			...
			case "HI":
				lineMessagingClient.pushMessage(new PushMessage(userId,
						activeService.getProductList().stream().map(TextMessage::new).collect(Collectors.toList())));
				break;
			case "YO":
	...
	private void sellItem(String itemID, String replyToken, String userID)
		...
		try {
			...
			// lineMessagingClient.replyMessage(new ReplyMessage(replyToken, new
			// TextMessage(actName + " 購買成功!"))).get();
			activeService.addOrder(itemID, userID);
			activeService.activeFinished(itemID);
		} catch (Exception e) {
