...
public class EchoService extends AbstractService {
  ...
  public void handleTextMessageEvent(MessageEvent<TextMessageContent> event) throws Exception {
    if ("hi".equalsIgnoreCase(event.getMessage().getText())) {
      ...
      lineMessagingClient.pushMessage(new PushMessage(event.getSource().getUserId(),
          actions.stream().map(TextMessage::new).collect(Collectors.toList())));
    } else {
      lineMessagingClient.replyMessage(
          new ReplyMessage(event.getReplyToken(), new TextMessage(event.getMessage().getText())))
          .get();
    }
  }
